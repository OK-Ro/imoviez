...
function Comedy() {
  const [movies, setMovies] = useState([]); // Change to 'movies' instead of 'randomMovies'
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = "https://node-mongo-mv85.onrender.com/api/movies/comedy";
        const response = await axios.get(apiUrl);
        console.log("API Response:", response.data); // Log the API response to see its structure
        setMovies(response.data || []); // Set 'movies' with the response data
        setIsLoading(false);
      } catch (error) {
...
const ComedyContainer = styled.div`
